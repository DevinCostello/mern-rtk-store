    ...
    if (data.type === "single") {
        ...
        return (
            <section className={styles.filter_group}>
                ...
                {data.filters.map((filter, index) =>
                    <div key={index} >
                        {/* hardcoded values VVV */}
                        <button className={price.lte === filter.lte ? styles.filterbtn_active : styles.filterbtn} 
                        type="checkbox" onClick={() => dispatch(setter({ filter }))}>{filter.text}</button>
                    </div>
                )}
